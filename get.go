...
// GetInt has the same behaviour as GetString but converts the response to int64
...
// GetInt has the same behaviour as GetString but converts the response to uint64.
func (c *ConfigFile) GetUint64(section string, option string) (value uint64, err error) {
	sv, err := c.GetString(section, option)
	if err == nil {
		value, err = strconv.ParseUint(sv, 10, 64)
		if err != nil {
			err = GetError{CouldNotParse, "uint64", sv, section, option}
		}
	}
